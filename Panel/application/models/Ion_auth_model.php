...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Reza
	 **/
	public function change_email($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_email_in_db = $this->db->update($this->tables['users'], $data, array($this->identity_column => $identity));
			if ($successfully_changed_email_in_db)
			{
				$this->trigger_events(array('post_change_email', 'post_change_email_successful'));
				$this->set_message('email_change_successful');
			}
			else
			{
				$this->trigger_events(array('post_change_email', 'post_change_email_unsuccessful'));
			...
			return $successfully_changed_email_in_db;
		}
	 ...
	 * @author Reza
	 **/
	public function change_username($identity, $old, $new)
	{
		$this->trigger_events('pre_change_username');
		...
		if ($query->num_rows() !== 1)
		{
			$this->trigger_events(array('post_change_username', 'post_change_username_unsuccessful'));
			$this->set_error('username_change_unsuccessful');
			return FALSE;
		}
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_username_in_db = $this->db->update($this->tables['users'], $data, array($this->identity_column => $identity));
			if ($successfully_changed_username_in_db)
			{
				$this->trigger_events(array('post_change_username', 'post_change_username_successful'));
				$this->set_message('username_change_successful');
			}
			else
			{
				$this->trigger_events(array('post_change_username', 'post_change_username_unsuccessful'));
			...
			return $successfully_changed_username_in_db;
		}
